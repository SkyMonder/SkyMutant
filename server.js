...
// Шифрование
let STORAGE_KEY;
function initStorageKey() {
  const hex = process.env.STORAGE_KEY_HEX;
  if (hex && hex.length === 64 && /^[0-9a-fA-F]{64}$/.test(hex)) {
    STORAGE_KEY = Buffer.from(hex, 'hex');
    console.log('✅ STORAGE_KEY загружен.');
  } else {
    STORAGE_KEY = crypto.randomBytes(32);
    console.warn('⚠️ STORAGE_KEY_HEX не задан. Сгенерирован временный ключ:');
    console.warn(STORAGE_KEY.toString('hex'));
  }
}
initStorageKey();
...
// ========== SkyCounter (счётчик) ==========
const SITES = ['skycitadel.onrender.com', 'serveratiuz.onrender.com']; // добавь свои домены

function add_visit_unique(site) {
  const today = new Date().toISOString().split('T')[0];
  const visits = dbGet('visits', today) || [];
  if (!visits.includes(site)) {
    visits.push(site);
    dbPut('visits', today, visits);
    return true;
  }
  return false;
}
...
app.get('/checkvizit', (req, res) => {
  ...
  const added = add_visit_unique(site);
  res.json({ status: added ? 'ok' : 'already_exists', site, date: new Date().toISOString().split('T')[0] });
});

app.get('/counter', (req, res) => {
  const today = new Date().toISOString().split('T')[0];
  const visits = dbGet('visits', today) || [];
  const all = SITES.map(site => ({ site, visits: visits.filter(v => v === site).length }));
  const total = visits.length;
  res.json({ today, total, details: all });
});
...
// ========== Поиск (SkySearch) ==========
async function fetchDuckDuckGo(q) { /* как раньше */ return []; }
async function fetchWikipedia(q) { /* как раньше */ return []; }
async function fetchGitHub(q) { /* как раньше */ return []; }
async function fetchStackExchange(q) { /* как раньше */ return []; }
async function fetchOpenStreetMap(q) { /* как раньше */ return []; }
async function fetchOpenLibrary(q) { /* как раньше */ return []; }
async function fetchQuote(q) { /* как раньше */ return []; }
async function fetchNews(q) { /* как раньше */ return []; }
async function fetchCocktail(q) { /* как раньше */ return []; }
    ...
    try {
      switch (msg.type) {
        ...
        case 'get_messages': {
          ...
          if (!chat || !chat.members.includes(currentUser)) ws.send(JSON.stringify({ type:'messages', messages:[] }));
          else ws.send(JSON.stringify({ type:'messages', messages: chat.messages || [] }));
          break;
        }
        case 'send_message': {
          const chat = dbGet('chats', msg.chatId);
          if (!chat || !chat.members.includes(currentUser)) return;
          const message = { from: currentUser, text: msg.text || '', time: Date.now() };
          chat.messages = chat.messages || [];
          chat.messages.push(message);
          dbPut('chats', msg.chatId, chat);
          for (const m of chat.members) {
            if (m !== currentUser && connections[m] && !chat.blocked?.includes(m)) {
              connections[m].send(JSON.stringify({ type:'message', chatId: msg.chatId, ...message }));
            }
          }
          break;
        }
        // остальные обработчики (create_private_chat, join_group, leave_group, звонки и т.д.) аналогичны
      }
